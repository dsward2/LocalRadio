...
@implementation HTTPStreamingServerResponse
...
{
    [self.audioDataArray addObject:audioData];
}
...
{
    // return a chunk of audio data
    NSData * resultData = NULL;
    
    if (self.audioDataArray.count > 0)
    {
        NSMutableData * firstData = self.audioDataArray.firstObject;
        
        if (firstData.length <= length)
        {
            resultData = firstData;
            
            [self.audioDataArray removeObject:firstData];
        }
        else
        {
            // this audio data is too large to return as a single chunk,
            // so split into two data objects, then replace existing array element with 2nd data and return 1st data
            NSRange subdataRange1 = NSMakeRange(0, length);
            NSRange subdataRange2 = NSMakeRange(length, firstData.length - length);
        
            NSData * subdata1 = [firstData subdataWithRange:subdataRange1];
            NSMutableData * subdata2 = [[firstData subdataWithRange:subdataRange2] mutableCopy];
            
            [self.audioDataArray replaceObjectAtIndex:0 withObject:subdata2];
            resultData = subdata1;
        }
    }
